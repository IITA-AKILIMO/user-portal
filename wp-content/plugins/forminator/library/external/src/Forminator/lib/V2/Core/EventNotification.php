...
namespace Forminator\Stripe\V2\Core;

use Forminator\Stripe\Events\UnknownEventNotification;
use Forminator\Stripe\Reason;
use Forminator\Stripe\RelatedObject;
use Forminator\Stripe\Util\EventNotificationTypes;
/**
...
abstract class EventNotification
{
    ...
    public function __construct($json, $client)
    {
        ...
        if (\array_key_exists('context', $json) && null !== $json['context']) {
            $this->context = \Forminator\Stripe\StripeContext::parse($json['context']);
        }
    ...
    {
        $response = $this->client->rawRequest('get', "/v2/core/events/{$this->id}", null, ['stripe_context' => $this->context], null, ['fetch_event']);
    ...
    protected function fetchRelatedObject()
    {
        ...
        $response = $this->client->rawRequest('get', $this->related_object->url, null, $options, null, ['fetch_related_object']);
        return $this->client->deserialize($response->body, \Forminator\Stripe\Util\Util::getApiMode($this->related_object->url));
    }
}
