 ...
 */
class IGDGoogle_Logger_File extends IGDGoogle_Logger_Abstract
{
  /**
  ...
  public function __construct(IGDGoogle_Client $client)
  {
    ...
    $file = $client->getClassConfig('IGDGoogle_Logger_File', 'file');
    ...
    $mode = $client->getClassConfig('IGDGoogle_Logger_File', 'mode');
    if (!$mode) {
      $this->mode = $mode;
    }

    $this->lock = (bool) $client->getClassConfig('IGDGoogle_Logger_File', 'lock');
    $this->file = $file;
  }
  ...
  protected function write($message)
  {
    if (is_string($this->file)) {
      ...
      throw new IGDGoogle_Logger_Exception('File pointer is no longer available');
    }
