 ...
 */
abstract class IGDGoogle_Logger_Abstract
{
  /**
   * Default log format
   */
  ...
  /**
   * @param IGDGoogle_Client $client  The current Google client
   */
  public function __construct(IGDGoogle_Client $client)
  {
    $this->setLevel(
        $client->getClassConfig('IGDGoogle_Logger_Abstract', 'level')
    );

    $format = $client->getClassConfig('IGDGoogle_Logger_Abstract', 'log_format');
    $this->logFormat = $format ? $format : self::DEFAULT_LOG_FORMAT;

    $format = $client->getClassConfig('IGDGoogle_Logger_Abstract', 'date_format');
   ...
   * @throws IGDGoogle_Logger_Exception If $level is invalid
   */
  protected function normalizeLevel($level)
  {
    ...
    throw new IGDGoogle_Logger_Exception(
