	...
	class Yuki_Button_Element extends Element {
			...
			return [
				( new Tabs() )
					...
					->addTab( 'content', __( 'Content', 'yuki' ), [
						( new Text( $this->getSlug( 'label' ) ) )
							->setLabel( __( 'Button Label', 'yuki' ) )
							->asyncText( ".{$this->slug}" )
							->setDefaultValue( __( 'Button', 'yuki' ) )
						,
						( new Text( $this->getSlug( 'link' ) ) )
							->setLabel( __( 'Button Link', 'yuki' ) )
					...
					->addTab(
						'style', __( 'Style', 'yuki' ),
						$this->getButtonStyleControls( $this->slug . '_', [
		...
		public function render( $attrs = [] ) {
            ...
            <a <?php $this->print_attribute_string( 'button' ); ?>>
				<?php echo esc_html( CZ::get( $this->getSlug( 'label' ) ) ) ?>
            </a>
			<?php
		}
	}
