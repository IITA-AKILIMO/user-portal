...
if ( !class_exists( 'Yuki_Header_Row' ) ) {
    class Yuki_Header_Row extends Row
    {
        ...
        {
            $classes = 'yuki-header-row yuki-header-row-' . $this->id;
            $attrs = [
                'class'    => $classes,
                'data-row' => $this->id,
            ];
        ...
        {
            return [ ( new Tabs() )->setActiveTab( 'content' )->addTab( 'content', __( 'Content', 'yuki' ), [ ( new Slider( $this->getRowControlKey( 'min_height' ) ) )->setLabel( __( 'Min Height', 'yuki' ) )->asyncCss( ".yuki-header-row-{$this->id} .container", [
                'min-height' => 'value',
            ] )->setDefaultValue( $this->getRowControlDefault( 'min_height', '80px' ) )->setDefaultUnit( 'px' )->enableResponsive()->setMin( 20 )->setMax( 300 ), new Separator(), ( new MultiSelect( $this->getRowControlKey( 'visibility' ) ) )->setLabel( __( 'Visibility', 'yuki' ) )->buttonsGroupView()->setChoices( [
                ...
                ( new Border( $this->getRowControlKey( 'border_top' ) ) )->setLabel( __( 'Top Border', 'yuki' ) )->asyncCss( ".yuki-header-row-{$this->id}", AsyncCss::border( 'border-top' ) )->enableResponsive()->displayBlock()->setDefaultBorder( ...$this->getRowControlDefault( 'border_top', [ 1, 'none', 'var(--yuki-base-200)' ] ) ),
                ( new Separator() )->setStyle( 'dashed' ),
                ( new Border( $this->getRowControlKey( 'border_bottom' ) ) )->setLabel( __( 'Bottom Border', 'yuki' ) )->asyncCss( ".yuki-header-row-{$this->id}", AsyncCss::border( 'border-bottom' ) )->enableResponsive()->displayBlock()->setDefaultBorder( ...$this->getRowControlDefault( 'border_bottom', [ 1, 'none', 'var(--yuki-base-200)' ] ) ),
                ( new Separator() )->setStyle( 'dashed' ),
                ( new BoxShadow( $this->getRowControlKey( 'shadow' ) ) )->setLabel( __( 'Box Shadow', 'yuki' ) )->asyncCss( ".yuki-header-row-{$this->id}", AsyncCss::shadow() )->enableResponsive()->displayBlock()->setDefaultShadow( ...$this->getRowControlDefault( 'shadow', [
                'rgba(44, 62, 80, 0.05)',
                '0px',
                '10px',
                '10px',
                ...
                ( new Separator() )->setStyle( 'dashed' ),
                ( new Background( $this->getRowControlKey( 'background' ) ) )->setLabel( __( 'Background', 'yuki' ) )->asyncCss( ".yuki-header-row-{$this->id}", AsyncCss::background() )->enableResponsive()->setDefaultValue( $this->getRowControlDefault( 'background', [
                'type'  => 'color',
