			...
			return [
				( new Tabs() )
					...
					->addTab( 'content', __( 'Content', 'yuki' ), [
						( new Toggle( $this->getSlug( 'prose' ) ) )
							->setLabel( __( 'Prose Style', 'yuki' ) )
							->openByDefault()
						,
						( new Separator() ),
						( new Editor( $this->getSlug( 'text' ) ) )
							->setLabel( __( 'Text Content', 'yuki' ) )
							->setDefaultValue( $this->getDefaultSetting( 'text', 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.' ) )
						,
						( new Separator() ),
						( new Slider( $this->getSlug( 'max_width' ) ) )
							->setLabel( __( 'Max Width', 'yuki' ) )
							->setDefaultValue( $this->getDefaultSetting( 'max_width', '' ) )
							->setOption( 'units', Utils::units_config( [
								[ 'unit' => 'px', 'min' => 0, 'max' => 1200 ],
								[ 'unit' => '%', 'min' => 0, 'max' => 100 ],
								[ 'unit' => 'vw', 'min' => 0, 'max' => 100 ],
							] ) )
						,
						( new ImageRadio( $this->getSlug( 'position' ) ) )
							->setLabel( __( 'Position', 'yuki' ) )
							->inlineChoices()
						...
						( new ImageRadio( $this->getSlug( 'alignment' ) ) )
							->setLabel( __( 'Text Alignment', 'yuki' ) )
							->inlineChoices()
							->setDefaultValue( $this->getDefaultSetting( 'alignment', 'left' ) )
							->setChoices( [
								'left'   => [
					...
					->addTab( 'style', __( 'Style', 'yuki' ), [
						...
						( new ColorPicker( $this->getSlug( 'colors' ) ) )
							...
							->addColor( 'initial', __( 'Link Initial', 'yuki' ), 'var(--yuki-primary-color)' )
							->addColor( 'hover', __( 'Link Hover', 'yuki' ), 'var(--yuki-primary-active)' )
						,
						( new Separator() ),
		...
		public function render( $attrs = [] ) {
            ...
            <div <?php $this->print_attribute_string( $id ); ?>>
                <div class="yuki-text-element-content">
					<?php echo wp_kses_post( $this->get( $this->getSlug( 'text' ), $settings ) ) ?>
                </div>
            </div>
			<?php
		}
