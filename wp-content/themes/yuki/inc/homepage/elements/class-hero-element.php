...
if ( ! class_exists( 'Yuki_Hero_Element' ) ) {
	...
	class Yuki_Hero_Element extends Element {
		...
		public function getControls() {
			return [
				( new Section() )
					->setLabel( __( 'Hero Content', 'yuki' ) )
					->setControls( $this->getContentControls() )
				,
				( new Section() )
					->setLabel( __( 'Hero Layout', 'yuki' ) )
					->setControls( $this->getLayoutControls() )
				,
				( new Section() )
					->setLabel( __( 'Hero Shape Divider', 'yuki' ) )
					->setControls( $this->getShapeDividerControls() )
				,
				( new Section() )
					->setLabel( __( 'Hero Style', 'yuki' ) )
					->setControls( $this->getStyleControls() )
				,

				( new Section( $this->getSlug( 'overlay' ) ) )
					->setLabel( __( 'Hero Overlay', 'yuki' ) )
					->enableSwitch( false )
					->setControls( [
						( new Background( $this->getSlug( 'overlay_background' ) ) )
							->setLabel( __( 'Overlay Color', 'yuki' ) )
							->setDefaultValue( [
								'type'  => 'color',
								'color' => 'rgba(0,0,0,0.45)',
							] )
						,
						(new Slider( $this->getSlug( 'overlay_opacity' ) ))
							->setLabel( __( 'Overlay Opacity', 'yuki' ) )
							->setMin( 0 )
							->setMax( 1 )
							->setDecimals( 2 )
			...
			return [
				( new Tabs() )
					...
					->addTab( 'content', __( 'Content', 'yuki' ), [
						( new Text( $this->getSlug( 'title' ) ) )
							->setLabel( __( 'Title', 'yuki' ) )
							->setDefaultValue( $this->getDefaultSetting( 'title', __( 'Build Your Awesome Business', 'yuki' ) ) )
						,

						( new Editor( $this->getSlug( 'description' ) ) )
							->setLabel( __( 'Description', 'yuki' ) )
							->setDefaultValue( $this->getDefaultSetting( 'description', 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua' ) )
						,

						( new Separator() ),
						( new Text( $this->getSlug( 'button_text' ) ) )
							->setLabel( __( 'Button Text', 'yuki' ) )
							->setDefaultValue( $this->getDefaultSetting( 'button-text', __( 'Get Started', 'yuki' ) ) )
						,
						( new Text( $this->getSlug( 'button_url' ) ) )
							->setLabel( __( 'Button URL', 'yuki' ) )
							->setDefaultValue( $this->getDefaultSetting( 'button-url', '#' ) )
						,
						( new Toggle( $this->getSlug( 'button_open_new_tab' ) ) )
							->setLabel( __( 'Open In New Tab', 'yuki' ) )
							->setDefaultValue( $this->getDefaultSetting( 'button-open-new-tab', 'no' ) )
						,
						( new Toggle( $this->getSlug( 'button_no_follow' ) ) )
							->setLabel( __( 'No Follow', 'yuki' ) )
							->setDefaultValue( $this->getDefaultSetting( 'button-no-follow', 'no' ) )
						,
						( new Separator() ),
						( new Radio( $this->getSlug( 'content_align' ) ) )
							->setLabel( __( 'Content Alignment', 'yuki' ) )
							->buttonsGroupView()
							->setDefaultValue( $this->getDefaultSetting( 'content-align', 'left' ) )
							->setChoices( [
								'left'   => __( 'Left', 'yuki' ),
					...
					->addTab( 'media', __( 'Media', 'yuki' ), [
						( new ImageUploader( $this->getSlug( 'media' ) ) )
							->setLabel( __( 'Media', 'yuki' ) )
							->setDefaultValue( $this->getDefaultSetting( 'media', [
								'url' => yuki_image_url( 'hero-media.png' ),
							] ) )
						,
						( new Slider( $this->getSlug( 'media_height' ) ) )
							->setLabel( __( 'Media Height', 'yuki' ) )
							->enableResponsive()
							->setMin( 200 )
							->setMax( 800 )
							->setDefaultUnit( 'px' )
							->setDefaultValue( $this->getDefaultSetting( 'media-height', '360px' ) )
						,
						( new Radio( $this->getSlug( 'media_align' ) ) )
							->setLabel( __( 'Media Alignment', 'yuki' ) )
							->buttonsGroupView()
							->setDefaultValue( $this->getDefaultSetting( 'media-align', 'center' ) )
							->setChoices( [
								'left'   => __( 'Left', 'yuki' ),
								'center' => __( 'Center', 'yuki' ),
								'right'  => __( 'Right', 'yuki' ),
		...
		protected function getLayoutControls() {
			return [
				( new Slider( $this->getSlug( 'min_height' ) ) )
					->setLabel( __( 'Min Height', 'yuki' ) )
					->enableResponsive()
					->setUnits( [
						[ 'unit' => 'px', 'min' => 100, 'max' => 800 ],
						[ 'unit' => 'vh', 'min' => 10, 'max' => 100 ],
					] )
					->setDefaultValue( $this->getDefaultSetting( 'min-height', '640px' ) )
				,
				( new Select( $this->getSlug( 'layout' ) ) )
					->setLabel( __( 'Layout', 'yuki' ) )
					->setDefaultValue( $this->getDefaultSetting( 'layout', 'content-media' ) )
					->setChoices( [
		...
		protected function getShapeDividerControls() {
			return [
				( new Select( $this->getSlug( 'shape_divider' ) ) )
					->setLabel( __( 'Shape Divider', 'yuki' ) )
					->setDefaultValue( 'tilt' )
					->setChoices( Utils::array_pluck( 'title', yuki_get_shapes() ) )
				,
				( new Toggle( $this->getSlug( 'shape_flip' ) ) )
					->setLabel( __( 'Flip', 'yuki' ) )
					->openByDefault()
				,
				( new Toggle( $this->getSlug( 'shape_invert' ) ) )
					->setLabel( __( 'Invert', 'yuki' ) )
					->closeByDefault()
				,
				( new Separator() ),
				( new Slider( $this->getSlug( 'shape_width' ) ) )
					->setLabel( __( 'Shape Divider Width', 'yuki' ) )
					->setMin( 0 )
					->setMax( 100 )
					->setDefaultUnit( '%' )
					->setDefaultValue( '100%' )
				,
				( new Slider( $this->getSlug( 'shape_height' ) ) )
					->setLabel( __( 'Shape Divider Height', 'yuki' ) )
					->setMin( 0 )
					->setMax( 200 )
					->setDefaultUnit( 'px' )
					->setDefaultValue( $this->getDefaultSetting( 'shape-height', '50px' ) )
				,
				( new Separator() ),
				( new ColorPicker( $this->getSlug( 'shape_color' ) ) )
					->setLabel( __( 'Shape Color', 'yuki' ) )
					->enableAlpha()
					->addColor( 'initial', __( 'Initial', 'yuki' ),
						$this->getDefaultSetting( 'title-initial', 'var(--yuki-base-100)' ) )
				,
			];
		}
		...
		protected function getStyleControls() {
			return [
				( new Background( $this->getSlug( 'background' ) ) )
					->setLabel( __( 'Background', 'yuki' ) )
					->enableResponsive()
					->setDefaultValue( $this->getDefaultSetting( 'background', [
						'type'  => 'color',
						'color' => 'var(--yuki-accent-active)'
					] ) )
				,
				( new Collapse() )
					->setLabel( __( 'Title & Description Style', 'yuki' ) )
					->setControls( [
						( new Typography( $this->getSlug( 'title_typography' ) ) )
							->setLabel( __( 'Title Typography', 'yuki' ) )
							->setDefaultValue( [
								'family'     => 'inherit',
								'fontSize'   => '2.75rem',
								'variant'    => '700',
						...
						,
						( new ColorPicker( $this->getSlug( 'title_color' ) ) )
							->setLabel( __( 'Title Color', 'yuki' ) )
							->enableAlpha()
							->addColor( 'initial', __( 'Initial', 'yuki' ),
								$this->getDefaultSetting( 'title-initial', 'var(--yuki-base-color)' ) )
						,
						( new Separator() ),
						( new Typography( $this->getSlug( 'description_typography' ) ) )
							->setLabel( __( 'Description Typography', 'yuki' ) )
							->setDefaultValue( [
								'family'     => 'inherit',
								'fontSize'   => '1rem',
								'variant'    => '400',
						...
						,
						( new ColorPicker( $this->getSlug( 'description_color' ) ) )
							->setLabel( __( 'Description Color', 'yuki' ) )
							->enableAlpha()
							->addColor( 'initial', __( 'Initial', 'yuki' ),
								$this->getDefaultSetting( 'description-initial', 'var(--yuki-base-100)' ) )
						,
					] )
				,
				( new Collapse() )
					->setLabel( __( 'Button Style', 'yuki' ) )
					->setControls( $this->getButtonStyleControls( $this->getSlug( 'button_' ), [
						'min-height'     => '48px',
		...
		public function enqueue_frontend_scripts( $id = null, $data = [] ) {
			...
			add_filter( 'yuki_filter_dynamic_css', function ( $css ) use ( $id, $settings ) {
				...
				// overlay
				$css[".yuki-hero.$id .hero-overlay"] = array_merge(
					Css::background( $this->get( $this->getSlug( 'overlay_background' ), $settings ) ),
				...
				if ( $shape_data ) {
					...
					$flip = $this->checked( $this->getSlug( 'shape_flip' ), $settings )
					        && in_array( 'shape_flip', $shape_data['options'] );
				...
				// title
				$css[".yuki-hero.$id .hero-title"] = array_merge(
					Css::typography( $this->get( $this->getSlug( 'title_typography' ), $settings ) ),
					Css::colors( $this->get( $this->getSlug( 'title_color' ), $settings ), [
						'initial' => 'color',
					] )
				);

				// description
				$css[".yuki-hero.$id .hero-description"] = array_merge(
					Css::typography( $this->get( $this->getSlug( 'description_typography' ), $settings ) ),
					Css::colors( $this->get( $this->getSlug( 'description_color' ), $settings ), [
						'initial' => 'color',
					] )
				);
		...
		protected function shape_divider( $settings ) {
			...
			$negative = $this->checked( $this->getSlug( 'shape_invert' ), $settings )
			            && in_array( 'shape_invert', $shape_data['options'] );
		...
		public function render( $attrs = [] ) {
            ...
            <div <?php $this->print_attribute_string( $id ); ?>>
				...
				<?php if ( $this->checked( $this->getSlug( 'overlay' ), $settings ) ): ?>
                    <div class="hero-overlay absolute w-full h-full left-0 top-0"></div>
				<?php endif; ?>

                <div class="<?php Utils::the_clsx( [
					'hero-container container mx-auto px-gutter relative z-[1]',
					'flex-row-reverse' => $this->get( $this->getSlug( 'layout' ), $settings ) === 'media-content',
				] ); ?>">
                    <div class="hero-content yuki-scroll-reveal p-gutter lg:p-0">
						<?php if ( $title != '' ): ?>
                            <h1 class="hero-title mb-gutter"><?php echo esc_html( $title ) ?></h1>
						<?php endif; ?>
						<?php if ( $desc != '' ): ?>
                            <div class="hero-description mb-gutter yuki-raw-html"><?php echo wp_kses_post( $desc ) ?></div>
						<?php endif; ?>
						<?php if ( $button_text != '' ): ?>
                            <div class="mt-double-gutter">
                                <a class="hero-button yuki-button" <?php $this->print_attribute_string( $this->getSlug( 'button' ) ); ?>>
					...
					<?php if ( ! empty( $media_attr ) ): ?>
                        <div class="hero-media yuki-scroll-reveal p-gutter">
                            <img <?php Utils::print_attribute_string( $media_attr ); ?> />
                        </div>
					<?php endif; ?>
                </div>

				<?php $this->shape_divider( $settings ) ?>
            </div>
			<?php
		}
	}
