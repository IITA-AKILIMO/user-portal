	...
	class Yuki_Features_Element extends Element {
		...
		public function getControls() {
			...
			return [
				...
				( new ColorPicker( $this->getSlug( 'card-color' ) ) )
					->setLabel( __( 'Colors', 'yuki' ) )
					->addColor( 'icon', __( 'Icon', 'yuki' ), 'var(--yuki-primary-active)' )
					->addColor( 'title', __( 'Text', 'yuki' ), 'var(--yuki-accent-active)' )
					->addColor( 'description', __( 'Description', 'yuki' ), 'var(--yuki-accent-color)' )
				,

				( new Collapse() )
					->setLabel( __( 'Card Style', 'yuki' ) )
					->setControls( $this->getCardStyleControls() )
				,

				( new Collapse() )
					->setLabel( __( 'Title & Description', 'yuki' ) )
					->setControls( [
						( new Typography( $this->getSlug( 'title-typography' ) ) )
						...
						,
						( new Typography( $this->getSlug( 'description-typography' ) ) )
						...
						,
						( new Separator() ),
						( new Slider( $this->getSlug( 'title-spacing' ) ) )
				...
				( new Collapse() )
					...
					->setControls( [
						( new Slider( $this->getSlug( 'media-size' ) ) )
							->setLabel( __( 'Size', 'yuki' ) )
							->setMin( 10 )
							->setMax( 200 )
							->setDefaultUnit( 'px' )
							->setDefaultValue( '28px' )
						,
						( new Slider( $this->getSlug( 'media-spacing' ) ) )
							->setLabel( __( 'Spacing', 'yuki' ) )
							->setMin( 10 )
							->setMax( 100 )
							->setDefaultUnit( 'px' )
		...
		protected function getFeaturesControls() {
			...
			$repeater = ( ( new Repeater( $this->getSlug( 'features' ) ) )
				->setLabel( __( 'Features', 'yuki' ) )
				->setTitleField( "<%= settings.title %>" )
				->setDefaultValue( $this->getDefaultSetting( 'features', [
					[
						'visible'  => true,
						'settings' => [
				...
				->setControls( [
					( new Radio( $this->getSlug( 'media-type' ) ) )
						->setLabel( __( 'Media Type', 'yuki' ) )
						->buttonsGroupView()
						->setDefaultValue( $this->getDefaultSetting( 'media-type', 'icon' ) )
						->setChoices( [
							'icon'  => __( 'Icon', 'yuki' ),
							'image' => __( 'Image', 'yuki' ),
						] )
					...
					( new Separator() ),
					( new Condition() )
						->setCondition( [ $this->getSlug( 'media-type' ) => 'icon' ] )
						->setControls( [
							( new Icons( $this->getSlug( 'icon' ) ) )
								->setLabel( __( 'Icon', 'yuki' ) )
								->setDefaultValue( $this->getDefaultSetting( 'icon', [
						...
						->setReverseControls( [
							( new ImageUploader( $this->getSlug( 'image' ) ) )
								->setLabel( __( 'Image', 'yuki' ) )
								->setDefaultValue( $this->getDefaultSetting( 'image', '' ) )
							,
						] )
					,
					( new Separator() ),
		...
		public function enqueue_frontend_scripts( $id = null, $data = [] ) {
			...
			add_filter( 'yuki_filter_dynamic_css', function ( $css ) use ( $id, $settings ) {
				...
				$css[".yuki-features.$id .feature-title"] = array_merge(
				...
				$css[".yuki-features.$id .feature-description"] = array_merge(
					Css::typography( $this->get( $this->getSlug( 'title-description' ), $settings ) )
				);
