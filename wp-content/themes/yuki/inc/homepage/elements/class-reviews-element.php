...
if ( ! class_exists( 'Yuki_Reviews_Element' ) ) {
	...
	class Yuki_Reviews_Element extends Element {
		...
		public function getControls() {
			return [
				...
				( new ColorPicker( $this->getSlug( 'card-color' ) ) )
					...
					->addColor( 'rating', __( 'Rating', 'yuki' ), '#ffc900' )
					->addColor( 'message', __( 'Message', 'yuki' ), 'var(--yuki-accent-color)' )
					->addColor( 'reviewer', __( 'Reviewer', 'yuki' ), 'var(--yuki-accent-active)' )
					->addColor( 'job-title', __( 'Job Title', 'yuki' ), 'var(--yuki-accent-color)' )
				,
				( new Collapse() )
					->setLabel( __( 'Typography', 'yuki' ) )
					->setControls( [
						( new Typography( $this->getSlug( 'message-typography' ) ) )
							->setLabel( __( 'Message Typography', 'yuki' ) )
							->setDefaultValue( [
								'family'     => 'inherit',
								'fontSize'   => '1rem',
								'variant'    => '400',
						...
						,
						( new Typography( $this->getSlug( 'reviewer-typography' ) ) )
							->setLabel( __( 'Reviewer Typography', 'yuki' ) )
							->setDefaultValue( [
								'family'        => 'inherit',
								'fontSize'      => '1rem',
								'variant'       => '600',
						...
						,
						( new Typography( $this->getSlug( 'job-title-typography' ) ) )
							->setLabel( __( 'Job Title Typography', 'yuki' ) )
							->setDefaultValue( [
								'family'     => 'inherit',
								'fontSize'   => '0.875rem',
								'variant'    => '400',
		...
		protected function getReviewsControl() {
			...
			$repeater = ( new Repeater( $this->getSlug( 'reviews' ) ) )
				->setLabel( __( 'Reviews', 'yuki' ) )
				->setTitleField( '<%= settings.reviewer %>' )
				->setDefaultValue( $this->getDefaultSetting( 'reviews', [
					[
						'visible'  => true,
						'settings' => [
							'message'   => $message,
				...
				->setControls( [
					( new Text( $this->getSlug( 'reviewer' ) ) )
						->setLabel( __( 'Reviewer', 'yuki' ) )
						->setDefaultValue( __( 'Someone', 'yuki' ) )
					,

					( new Text( $this->getSlug( 'job-title' ) ) )
						->setLabel( __( 'Job Title', 'yuki' ) )
						->setDefaultValue( 'CEO' )
					,

					( new Slider( $this->getSlug( 'rating' ) ) )
						->setLabel( __( 'Rating', 'yuki' ) )
						->setMin( 0 )
						->setMax( 5 )
						->setDefaultValue( 5 )
					...
					( new Editor( $this->getSlug( 'message' ) ) )
						->setLabel( __( 'Message', 'yuki' ) )
		...
		public function enqueue_frontend_scripts( $id = null, $data = [] ) {
			...
			add_filter( 'yuki_filter_dynamic_css', function ( $css ) use ( $id, $settings ) {
				...
				$css[".yuki-reviews.$id .review-message"] = array_merge(
				...
				$css[".yuki-reviews.$id .review-reviewer"] = array_merge(
					Css::colors( $this->get( $this->getSlug( 'card-color' ), $settings ), [
						'reviewer' => 'color',
					] ),
				...
				$css[".yuki-reviews.$id .review-job-title"] = array_merge(
					Css::colors( $this->get( $this->getSlug( 'card-color' ), $settings ), [
						'job-title' => 'color',
					] ),
                    ...
                    <div class="review-item-wrap p-half-gutter flex-grow">
                        <div class="review-item h-full yuki-scroll-reveal">
                            ...
                            <div class="review-message py-half-gutter yuki-raw-html">
								<?php echo wp_kses_post( $review['message'] ) ?>
                            </div>
                            ...
                            <span class="review-job-title">
	                            <?php echo esc_html( $review['job-title'] ) ?>
	                        </span>
                        </div>
                    </div>
					<?php
