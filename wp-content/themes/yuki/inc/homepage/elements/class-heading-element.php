	...
	class Yuki_Heading_Element extends Element {
		...
		public function getControls() {
			return [
				( new Text( $this->getSlug( 'title' ) ) )
					...
					->setDefaultValue( $this->getDefaultSetting( 'title', __( 'Awesome title', 'yuki' ) ) )
				,

				( new Text( $this->getSlug( 'sub-title' ) ) )
					->setLabel( __( 'Sub Title', 'yuki' ) )
					->setDefaultValue( $this->getDefaultSetting( 'sub-title', 'Lorem ipsum is placeholder text commonly used' ) )
				,

				( new Select( $this->getSlug( 'title-tag' ) ) )
					->setLabel( __( 'Widget Title Tag', 'yuki' ) )
					->setDefaultValue( $this->getDefaultSetting( 'title-tag', 'h3' ) )
					->setChoices( [
						'h1'   => 'H1',
						'h2'   => 'H2',
						'h3'   => 'H3',
				...
				( new Separator() ),
				( new ColorPicker( $this->getSlug( 'colors' ) ) )
					->setLabel( __( 'Colors', 'yuki' ) )
					->enableAlpha()
					->addColor( 'title', __( 'Title', 'yuki' ), 'var(--yuki-accent-active)' )
					->addColor( 'sub-title', __( 'Sub Title', 'yuki' ), 'var(--yuki-accent-color)' )
				,
				( new ImageRadio( $this->getSlug( 'alignment' ) ) )
					->setLabel( __( 'Alignment', 'yuki' ) )
					->inlineChoices()
				...
				,
				( new Typography( $this->getSlug( 'sub-title-typography' ) ) )
					->setLabel( __( 'Sub Title Typography', 'yuki' ) )
					->setDefaultValue( [
						'family'     => 'inherit',
						'fontSize'   => [
							'desktop' => '1rem',
		...
		public function enqueue_frontend_scripts( $id = null, $data = [] ) {
			...
			add_filter( 'yuki_filter_dynamic_css', function ( $css ) use ( $id, $settings ) {
				...
				$css[".yuki-heading-element.$id .heading-title"] = array_merge(
				...
				$css[".yuki-heading-element.$id .heading-sub-title"] = array_merge(
					Css::colors( $this->get( $this->getSlug( 'colors' ), $settings ), [
						'sub-title' => 'color',
					] ),
		...
		public function render( $attrs = [] ) {
            ...
            <div <?php $this->print_attribute_string( $id ); ?>>
				...
				echo esc_html( $this->get( $this->getSlug( 'title' ), $settings ) );
				echo '</' . $tag . '>';
				?>
                <p class="heading-sub-title">
					<?php echo esc_html( $this->get( $this->getSlug( 'sub-title' ), $settings ) ) ?>
                </p>
            </div>
			<?php
		}
