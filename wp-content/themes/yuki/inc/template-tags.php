		...
		<?php foreach ( $structure as $item ): ?>
			<?php if (
				$item === 'thumbnail' && ( has_post_thumbnail() || (
						$options->checked( 'yuki_' . $id . '_thumbnail_use_fallback' )
						&& CZ::hasImage( 'yuki_post_featured_image_fallback' )
					) )
			): ?>
				<?php
				if ( $content_open ) {
                ...
                <a href="<?php the_permalink() ?>" class="card-thumbnail entry-thumbnail last:mb-0">
					...
					if ( has_post_thumbnail() ) {
						the_post_thumbnail( 'yuki-thumbnail', [
							'class' => 'w-full h-full',
						] );
					} else {
		...
		foreach ( $elements as $element ) {
			...
			if ( $element === 'thumbnail' ) {
				$css["$scope_selector .entry-thumbnail"] = array_merge(
					...
					Css::dimensions( $options->get( 'yuki_' . $id . '_thumbnail_radius', $settings ), 'border-radius' ),
					Css::shadow( $options->get( 'yuki_' . $id . '_thumbnail_shadow', $settings ) )
				);
			}
	...
	function yuki_get_preloader( $id ) {
		$presets = [
			...
			'preset-6'  => [
				...
				'keyframes' => [
					'pieFill' => [
						'0%, 19%'   => [ 'border-color' => 'var(--yuki-preloader-accent) var(--yuki-preloader-accent) var(--yuki-preloader-accent) var(--yuki-preloader-accent)' ],
						'20%, 39%'  => [ 'border-color' => 'var(--yuki-preloader-primary) var(--yuki-preloader-accent) var(--yuki-preloader-accent) var(--yuki-preloader-accent)' ],
						'40%, 59%'  => [ 'border-color' => 'var(--yuki-preloader-primary) var(--yuki-preloader-primary) var(--yuki-preloader-accent) var(--yuki-preloader-accent)' ],
						'60%, 79%'  => [ 'border-color' => 'var(--yuki-preloader-primary) var(--yuki-preloader-primary) var(--yuki-preloader-primary) var(--yuki-preloader-accent)' ],
						'80%, 100%' => [ 'border-color' => 'var(--yuki-preloader-primary) var(--yuki-preloader-primary) var(--yuki-preloader-primary) var(--yuki-preloader-primary)' ],
					],
				],
			],
			'preset-7'  => [
