...
if ( !trait_exists( 'Yuki_Post_Structure' ) ) {
    ...
    trait Yuki_Post_Structure
    {
        ...
        protected function getExcerptControls( $id, $defaults )
        {
            ...
            ] )->setControls( [ ( new Slider( 'yuki_' . $id . '_excerpt_length' ) )->setLabel( __( 'Length', 'yuki' ) )->setMin( 10 )->setMax( 300 )->setDefaultUnit( false )->setDefaultValue( $defaults['length'] ), new Separator(), ( new Text( 'yuki_' . $id . '_excerpt_more_text' ) )->setLabel( __( 'More Text', 'yuki' ) )->setDefaultValue( $defaults['more'] ) ] ), new Separator() ];
            $controls = array_merge( $controls, [ yuki_upsell_info_control( __( 'More options in our %sPro Version%s', 'yuki' ) ), ( new Placeholder( 'yuki_' . $id . '_excerpt_typography' ) )->setDefaultValue( $defaults['typography'] ), ( new Placeholder( 'yuki_' . $id . '_excerpt_color' ) )->addColor( 'initial', $defaults['color'] ) ] );
            return $controls;
        }
